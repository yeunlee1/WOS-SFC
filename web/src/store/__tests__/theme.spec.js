...
//
// 검증 목적:
//  1) THEMES 배열에 frost 포함 + 3개 테마 모두 등록됨 (UI 옵션 누락 방지)
//  2) setTheme: 알 수 없는 값 → fallback 'frost' (신규 사용자 기본값)
//  3) setTheme: 유효한 3개 테마 모두 통과
...
describe('store theme — THEMES 정책 + setTheme/_initTheme 회귀 가드', () => {
  ...
  it('THEMES 배열은 3개 테마 (frost/spring/daylight), frost-first', async () => {
    const { THEMES } = await import('../index.js');
    // frost = 메인 다크, spring = 벚꽃 다크, daylight = 백야 라이트. anthropic/dark 폐기.
    expect(THEMES).toEqual(['frost', 'spring', 'daylight']);
  ...
  it('_initTheme: 유효한 frost/spring/daylight localStorage 값은 유지', async () => {
    for (const t of ['frost', 'spring', 'daylight']) {
      vi.resetModules();
      localStorage.clear();
  ...
  it('setTheme: 유효한 3개 테마 모두 적용 + localStorage 동기화', async () => {
    const { useStore } = await import('../index.js');
  ...
  it('setTheme: daylight 새로고침 재현 — 저장 후 모듈 재로드하면 daylight 유지', async () => {
    const { useStore } = await import('../index.js');
    useStore.getState().setTheme('daylight');
    expect(localStorage.getItem('wos-theme')).toBe('daylight');
    // 새로고침 시뮬레이션: 모듈 재로드 → _initTheme 재실행
    vi.resetModules();
    const reloaded = await import('../index.js');
    expect(reloaded.useStore.getState().theme).toBe('daylight');
  });
