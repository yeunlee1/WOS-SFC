...
// 4. 단순 maxRTT 대신 median(validRtts) 기반 grace 계산 (Sec-C2-strong):
//    단일 악성 클라이언트가 임계(OUTLIER_RTT_MS) 직전 응답으로 maxRTT를 부풀려도
//    median은 50%+ 정상 가정 하에 영향 거의 없음.
// 5. startedAt = Date.now() + min(median * 2 + MARGIN, MAX_GRACE)
//
...
export class ReadyNegotiationService {
  ...
  async negotiateStartedAt(server: Server): Promise<number> {
    ...
    // outlier (RTT > OUTLIER_RTT_MS) 및 응답 없음(null)은 FALLBACK_RTT로 대체.
    // 3회차 Sec-C2-strong: 단순 maxRTT 대신 median(rtts) 기반 grace 계산.
    // 단일 악성 클라이언트가 OUTLIER_RTT_MS 직전(예: 450ms) 응답으로 maxRTT를 부풀려도
    // median은 50%+ 정상 클라이언트 기반으로 결정되므로 robust.
    ...
    const medianRtt = ReadyNegotiationService.calcMedian(rtts);
    const successCount = probeResults.filter((r) => r !== null).length;

    const computed = medianRtt * 2 + ReadyNegotiationService.STARTUP_GRACE_MARGIN_MS;
    const grace = Math.min(computed, ReadyNegotiationService.MAX_STARTUP_GRACE_MS);

    this.logger.log(
      `negotiateStartedAt: ${successCount}/${sockets.length} ack, medianRtt=${medianRtt}ms, grace=${grace}ms`,
    );
   ...
   */
  private static calcMedian(values: number[]): number {
